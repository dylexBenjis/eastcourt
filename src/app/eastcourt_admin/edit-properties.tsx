...
const[deletedFiles, setDeletedFiles]=useState<{
    ...
    imageUrl?: string | undefined;
                ...
                useEffect(()=>{
console.log(files)                 
                  if(label=='FOR_SALE'){setLabelValue('sale')}else{setLabelValue('rent')}
                },[])
 ...
 //handle select images button click
  ...
  const handleImageUpload = (e:React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files) {
      ...
      setNewFiles([...new_files, ...imageUrls.map((url, index) => ({ name: `${uuid}${newImages[index].name}`, imageUrls: url, file: newImages[index] }))])
    }     
...
const removeImage = (index: number) => {
  if(files){setDeletedFiles([...deletedFiles, files[index]])}
  setFiles((prevImages) => prevImages?.filter((_, i) => i !== index));
}


    //work on both new and old property image and video files
    const [updatedFiles, setUpdatedFiles] = useState<{type?: string | undefined; imageUrl?: string | undefined;}[]>([]);
    ...
    const [propertyData, setPropertyData] = useState<Property>({})
      const check_changes=  ()=>{
            ...
            if(label!==edit_list.label){
              if (labelValue === "sale" ) {setPropertyData(prev=>({...prev, label:"FOR__SALE"}))}
              else{setPropertyData(prev=>({...prev, label:"FOR_RENT"}))}
            }
 ...
 useEffect(()=>{
  ...
  const handlePropertyUpdate = async (e: React.FormEvent) => {
    ...
    return(
<div className='my-5'>
{edit_list && <div className="relative container px-4 py-6 md:px-6 md:py-8 ">
      

    {/*post property ui*/}
      <div className="mb-6">
        <h1 className="text-3xl font-bold" onClick={()=>{console.log(edit_list)}}>Editing a Property</h1>
        <p className="text-muted-foreground">Edit the details of this property - {edit_list.title}</p>
      </div>

      <Card>
        <CardContent className="p-6">
          <form className="space-y-8" onSubmit={handlePropertyUpdate}  ref={formRef} >
             Basic Information 
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input id="title" value={title} onChange={(event)=>{setTitle(event.target.value)}} placeholder="e.g. Modern Apartment with City View"/>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="price">Price (&#x20A6;)</Label>
                  <Input id="price" type="number" value={price} onChange={(event)=>{setPrice(parseInt(event.target.value))}} placeholder="e.g. 350000" />
                </div>
              </div>

              <div className="space-y-2">
                <Label>Listing Type</Label>
                <RadioGroup defaultValue={label=='FOR_SALE'?'sale':'rent'} className="flex gap-4" onValueChange={(value)=>{setLabelValue(value)}}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="sqft" type="number" value={sqft} onChange={(event)=>{setSQFT(event?.target.value)}} placeholder="e.g. 1500" />
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={parking} onValueChange={(value)=>{setParking(value)}}>
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-semibold">Location</h2>
                            ...
                            <div className="space-y-2">
                <Label>Include coordinate in order to get precise location on google map</Label>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input id="address" value={address} onChange={(event)=>{setAddress(event.target.value)}} placeholder="Street address" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="location" value={location} onChange={(event)=>{setLocation(event.target.value)}} placeholder="latitude, longitude" />
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 mt-3">
                    {new_files.map((image, index:number) => (
                      <div key={index} className="flex relative aspect-square rounded-md">
                        {image.file&&(image.file.type.startsWith('image/')&&<img
                          src={image.imageUrls || "/placeholder.svg"}
                          alt={`Property image ${index + 1}`}
                          className="h-full w-full rounded-md object-contain"
                        />)}
                        {image.file&&(image.file.type.startsWith('video')&&(<video
                          src={image.imageUrls}
                          controls
                          autoPlay
                          ...
                          muted
                        />))}
                        <button type='button' onClick={()=>{remove_newImage(index)}} className="absolute cursor-pointer justify-center items-center right-1 bg-red-600 h-[25px] w-[25px] rounded-full text-white">x</button>
                      </div>
                    ))}
                  </div></div>
                )}
                ...
                {files  && (<>
                  ...
                  <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                    {files.map((image:any, index:any) => (
                      <div key={index} className="flex relative aspect-square rounded-md">
                        {image.type&&(image.type.startsWith('image')&&<img
                          src={image.imageUrl || "/placeholder.svg"}
                          alt={`Property image ${index + 1}`}
                          className="h-full w-full rounded-md object-contain"
                        />)}
                        {image.type&&(image.type.startsWith('video')&&(<video
                          src={image.imageUrl}
                          controls
                          autoPlay
                          className="h-full w-full rounded-md"

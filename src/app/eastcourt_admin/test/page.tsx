...
import { Property } from "@/src/lib/upload-properties";

export default function PropertyDetailsPage() {
  const param = useSearchParams();
  const id = param.get("id");
  const [approved_Listings, setApprovedListings] = useState<Property[]>(
    JSON.parse(sessionStorage.getItem("approved-listings") || "[]")
  );
  const [unapproved_Listings, setUnApprovedListings] = useState<Property[]>(
    JSON.parse(sessionStorage.getItem("my-listings") || "[]")
  );

  let property = approved_Listings.find((item) => item.id === id);
  if (property == undefined) {
    property = unapproved_Listings.find((item) => item.id === id);
  }

  //getting similar properties
  //  const[similar_properties, setSimilar_properties] = useState<Property[]>([])
  //  useEffect(()=>{
  //   setSimilar_properties(approved_Listings.)
  //  })
              ...
              <Card key={i} className="overflow-hidden">
                <div className="relative">
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    className="absolute top-3 right-3 bg-background/80 rounded-full"
                  >
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex justify-between text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center">
                      ...
                      <span>
                        {i === 1 ? "1,200" : i === 2 ? "4,500" : "2,800"} sq ft
                      </span>
                    </div>
                  </div>
                </CardContent>

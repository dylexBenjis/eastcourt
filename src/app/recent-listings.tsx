...
export function RecentListings() {
    ...
    try {
      ...
      if (nextBatch) {
        ...
        q = query(
          ...
          limit(document_size + docincrement)
        );
      } else {
      ...
      if (!querySnapshot.empty) {
        ...
        setRecentListings((prev) =>
          nextBatch ? [...prev, ...listings] : listings
        );
  ...
  useEffect(() => {
    console.log("rr", recent_listing, "hasmore", hasMore);
  }, [recent_listing]);
  ...
  return (
    <>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {recent_listings_loading &&
          Array.from({ length: expectedListings }).map((_, index) => (
            <PropertyCardSkeleton key={index} />
          ))}
      </div>
